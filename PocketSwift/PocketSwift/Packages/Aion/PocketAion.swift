...
public class PocketAion: Pocket, PocketPlugin {
    ...
    init(devID: String, netIds: [String], maxNodes: Int = 5, requestTimeOut: Int = 1000, schedulerProvider: SchedulerProvider) {
        super.init(devID: devID, network: self.NETWORK, netIds: netIds, maxNodes: maxNodes, requestTimeOut: requestTimeOut, schedulerProvider: schedulerProvider)
    ...
    public convenience init(devID: String, netIds: [String], maxNodes: Int = 5, requestTimeOut: Int = 1000) {
        self.init(devID: devID, netIds: netIds, maxNodes: maxNodes, requestTimeOut: requestTimeOut, schedulerProvider: .main)
    }
    
    public convenience init(devID: String, netID: String, maxNodes: Int = 5, requestTimeOut: Int = 1000) {
        let netIds: [String] = [netID]
        self.init(devID: devID, netIds: netIds, maxNodes: maxNodes, requestTimeOut: requestTimeOut)
    }
    
    public func network(_ netID: String) -> Network {
    ...
    func createWallet(network: String, netID: String, data: [AnyHashable : Any]?) throws -> Wallet {
        ...
        return Wallet(address: address, privateKey: privateKey, network: network, netID: netID, data: data)
    }
    ...
    func importWallet(privateKey: String, address: String?, network: String, netID: String, data: [AnyHashable : Any]?) throws -> Wallet {
        ...
        if (self.jsContext.evaluateScript("var account = aionInstance.eth.accounts.privateKeyToAccount('\(privateKey)')") != nil) {
            ...
            return Wallet(address: publicKey, privateKey: privateKey, network: network, netID: netID, data: data)
        }

        throw PocketError.walletImport(message: "Failed to create account js object")
    }
    
//    override public func createWallet(network: String, netID: String, data: [AnyHashable: Any]?) throws -> Wallet {
//        guard let account = self.jsContext.evaluateScript("aionInstance.eth.accounts.create()")?.toObject() as? [AnyHashable: Any] else {
//            throw PocketError.walletCreation(message: "Failed to create account")
//        }
//
...
//    }
//
//    override public func importWallet(privateKey: String, address: String?, network: String, netID: String, data: [AnyHashable : Any]?) throws -> Wallet {
//        guard let publicKey = address else {
//            throw PocketError.walletImport(message: "Invalid public key")
//        }
//
